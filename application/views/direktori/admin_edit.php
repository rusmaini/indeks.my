...
<div class="row">
	<div class="col-sm-8">
		<!-- heading -->
		<?= ($id==0)? '<h4>Tambah Item</h4>':'<h4>Edit Item</h4>'; ?>
		
		<hr />
		<?=form_open_multipart('direktori/urus_edit/'.$id,array('class'=>'form form-horizontal')); ?>  
	  	<div class="form-group">
		    <label class="col-sm-3 control-label">Pilih Pemilik Iklan <span class="merah">*</span></label>
		    <div class="col-sm-6">
		      <?=form_dropdown('user_id',$user,(isset($item))?$item['user_id']:set_value('user_id'),' class="form-control"')?>
	          <?= form_error('user_id'); ?>
		    </div>
	  	</div>
		...
		<?php if(isset($item)):?>
			...
			<?php if($item['gambar']): ?>
				...
				<div class="row" align="center">
					...
					<div class="col-sm-4">
						<?=anchor('direktori/urus_gambarutama_delete/'.$id,'<span class="glyphicon glyphicon-trash"></span> Padam',array('onclick'=>'return confirm(\'Gambar akan dihapuskan. Teruskan?\')'))?>
					</div>
				</div>
			<?php endif; ?>	
			<br />
			...
			<p>Gambar Tambahan:</p>
			<?=form_open_multipart('direktori/upload_gambar/'.$id,array('name'=>'formupload','class'=>'form form-inline')); ?>  
			<div class="form-group">
			    <input type="file" name="userfile" />
		      	<?=form_hidden('id',$id)?>
			</div>
			...
			<?php if(isset($gambar)):?>
				...
				<?php foreach($gambar as $g): ?>
					...
					<div class="row" align="center">
						...
						<div class="col-sm-4">
							<?=anchor('direktori/urus_gambarextra_delete/'.$id.'/'.$g['id'],'<span class="glyphicon glyphicon-trash"></span> Padam',array('onclick'=>'return confirm(\'Gambar akan dihapuskan. Teruskan?\')'))?>
						</div>
					</div>
					<br />
				<?php endforeach; ?>
